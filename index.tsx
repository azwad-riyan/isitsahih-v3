    ...
    <div className="container">
      ...
      <main role="main">
        ...
        {!loading && share && (
          <>
            {share.claim && (
              <div style={{ background: "rgba(13,148,136,0.07)", border: "1px solid rgba(13,148,136,0.22)", borderRadius: 12, padding: "1rem 1.25rem", margin: "0.5rem 0 1.25rem" }}>
                <p style={{ fontSize: "0.68rem", fontWeight: 700, textTransform: "uppercase", letterSpacing: "0.06em", color: "#0d9488", margin: "0 0 0.5rem", opacity: 0.85 }}>
                  {language === "Bangla" ? "যাচাইকৃত দাবি" : "Claim being verified"}
                </p>
                <p className="description" style={{ fontStyle: "italic", margin: 0 }}>"{share.claim}"</p>
              </div>
            )}
            ...
            <div
              ...
              style={{ background: "rgba(13,148,136,0.05)", border: "1px solid rgba(13,148,136,0.15)", borderRadius: 10, padding: "0.6rem 1rem", fontSize: "0.78rem", margin: "1rem 0 0.25rem", opacity: 0.75 }}
            >
...
const App = () => {
  ...
  const runVerify = async (trimmed: string) => {
    setLoading(true);
    setResult(null);
    setError("");
    setShareUrl("");
  ...
  // Auto-verify when the page is opened with ?q= (supports the SearchAction schema).
  useEffect(() => {
    const q = (() => { try { return new URLSearchParams(window.location.search).get("q")?.trim() ?? ""; } catch { return ""; } })();
    if (q.length >= MIN_LEN && q.length <= MAX_LEN) runVerify(q);
  ...
  const Footer = () => (
    <footer className="footer">
      ...
      <div className="support-section">
        ...
        <p className="copyright">© 2026 Is It Sahih. {language === "Bangla" ? "সর্বস্বত্ব সংরক্ষিত।" : "All rights reserved."}</p>
        <p style={{ fontSize: "0.72rem", opacity: 0.6, marginTop: "0.35rem" }}>
          <a href="/about.html" style={{ color: "inherit" }}>{language === "Bangla" ? "পরিচিতি" : "About"}</a>
          {" · "}
          <a href="/privacy.html" style={{ color: "inherit" }}>{language === "Bangla" ? "গোপনীয়তা নীতি" : "Privacy Policy"}</a>
        </p>
      </div>
    </footer>
  );
